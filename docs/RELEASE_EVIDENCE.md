...
| Offline test suite | 92 tests passed on 2026-08-10, including concurrent publication attempts, exception-safe claim resolution, ambiguous VPS outcomes, sealed-revision-to-request binding, legacy validation, Chrome reconnect/interrupt cleanup, constrained publication states, and anchored social metadata readback; rerun after every release-gate change | verified |
...
| --- | --- | --- |
| Personal website | New-project `acp` execution receives explicit VPS business success and the article is visible | pending user-approved live run |
