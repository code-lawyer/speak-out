...
Article publication sends one manifest-verified snapshot of the approved MDX, WeChat HTML, and cover to the configured VPS. Social publication reads copy from a verified snapshot and exposes a private, manifest-bound MP4 copy only to the selected creator page in visible Chrome. The temporary copy and its non-secret identity/hash sidecar stay inside the ignored Product staging directory. They enter the retained state before file selection and remain while Chrome is still uploading, because deleting or moving the source aborts asynchronous platform uploads. Removal requires explicit remote completion, explicit failure after the current attempt is acknowledged, or proof that upload never started. A retry re-derives the approved SHA-256 and revision digest from the sealed artifact manifest; the sidecar is never an integrity trust root.
